...
def test_create_folder_returns_201(cleanup_folder):
    ...
    response = requests.put(
        f"{API_BASE}", headers=HEADERS, params={"path": FOLDER_PATH}
    )
    assert (
...
def test_folder_appears_in_file_list(cleanup_folder):
    ...
    # Ждём, пока ЯндексДиск обновит индекс
    time.sleep(1)

    # Получаем список корневой директории
    response = requests.get(
        f"{API_BASE}", headers=HEADERS, params={"path": "disk:/", "limit": 1000}
    )
    ...
    assert (
        FOLDER_NAME in items
    ), f"Созданная папка '{FOLDER_NAME}' не найдена в списке: {items}"
...
def test_get_nonexistent_folder():
    ...
    response = requests.get(
        f"{API_BASE}", headers=HEADERS, params={"path": "disk:/nonexistent_folder"}
    )
    assert (
